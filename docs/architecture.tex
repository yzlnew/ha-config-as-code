...
\begin{tikzpicture}[
    ...
    \node[anthropicWideCard, draw=tealStroke, fill=tealFill] (resources) at (5.5,-7.6)
        {\anthropicLabel{tealText}{tealSubtext}{Home Assistant 配置资源}{Automations \textperiodcentered\ Scenes \textperiodcentered\ Dashboard \textperiodcentered\ Groups \textperiodcentered\ Helpers \textperiodcentered\ Entity / Device / Area Registry}};
    ...
    \draw[anthropicEdge] (resources.south) -- ++(0,-0.9)
        node[edgeLabel, above right, pos=0.5] {委托给集成}
        coordinate (bus);
    ...
    \draw[anthropicDashedEdge] (adaptive.south) |- node[edgeLabel, above, pos=0.75] {色温 / 亮度曲线} (lights.west);
    ...
    \begin{scope}[on background layer]
        \node[
            ...
            label={[anthropicGroupTitle, anchor=south west]north west:{配置即代码 (Git + Python)}}
        ] {};
        \node[
            anthropicGroup,
            fit=(rest) (ws) (resources),
            label={[anthropicGroupTitle, anchor=south west]north west:{Home Assistant 核心}}
        ] {};
        \node[
            anthropicGroup,
            fit=(adaptive) (matter) (homekit) (xiaomi) (esphome) (genericcam),
            label={[anthropicGroupTitle, anchor=south west]north west:{集成层 (core + HACS)}}
        ] {};
        \node[
            anthropicGroup,
            fit=(lights) (threadbr) (miji) (trmnl) (cams),
            label={[anthropicGroupTitle, anchor=south west]north west:{物理设备 / 外部系统}}
        ] {};
    \end{scope}
